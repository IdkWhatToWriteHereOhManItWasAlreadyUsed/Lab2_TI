		...
		private: System::Void îòêğûòüToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e) 
		{
			if (openFileDialog1->ShowDialog() == Windows::Forms::DialogResult::OK) 
			{
				...
				if (file) {
					...
					if (size > 3000000)
					{
						MessageBox::Show("Ñëèøêîì áîëüøîé ôàéë (âûáèğàéòå íå áîëåå 3 ìåãàáàéò)");
						return;
					}				
				}
				else {
...
private: System::Void îĞàçğàáîò÷èêåToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e) 
{
	...
	void saveBitsToFile(const std::vector<uint8_t>& bitArray, const std::string& filename) {
		if (bitArray.size() % 8 != 0) {
			throw std::invalid_argument("Ğàçìåğ ìàññèâà áèòîâ äîëæåí áûòü êğàòåí 8");
		}

		std::ofstream outFile(filename, std::ios::binary);
		if (!outFile) {
			throw std::runtime_error("Íå óäàëîñü îòêğûòü ôàéë äëÿ çàïèñè: " + filename);
		}
		...
		for (size_t i = 0; i < bitArray.size(); i += 8) {
			...
			for (size_t j = 0; j < 8; ++j) {
				...
				if (bitArray[bitPos] > 1) {
					throw std::invalid_argument("İëåìåíòû ìàññèâà äîëæíû áûòü 0 èëè 1");
				}
		...
		if (!outFile.good()) {
			throw std::runtime_error("Îøèáêà çàïèñè â ôàéë: " + filename);
		}
	}
